...
bool InputHandler::Startup(Window * a_InputWindow) {
	m_MainWindow = a_InputWindow;
	HRESULT hr;

	hr = DirectInput8Create(a_InputWindow->m_HInstance, DIRECTINPUT_VERSION, IID_IDirectInput8, (VOID**)&m_Di, NULL);
	if (FAILED(hr)) {
		return false;
	}
